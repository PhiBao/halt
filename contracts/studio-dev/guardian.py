...
class HaltGuardian(gl.contract.Contract):
    owner: Address
    vault: Address
    ...
    # Enforcement note (RC port): the vault derives freeze state live from this
    # guardian via a cross-contract view on every spend (fail-closed), so no
    # internal emit message is required for freeze/unfreeze to take effect.
    # This avoids the RC message-fee allocation model entirely.
...
"""
        ...
        def evaluate():
            ...
            shape = {
                ...
                "reasons": [str(x) for x in parsed.get("reasons", [])[:6]],
            }
            if prior is not None:
                shape["overturn"] = bool(parsed.get("overturn", False))
            return _dumps(shape)

        principle = (
            "The `violation` and `severity` fields must be exactly identical."
            + (" The `overturn` field must be exactly identical." if prior is not None else "")
