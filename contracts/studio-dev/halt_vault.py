...
def _valid_day_key(s) -> bool:
    if not isinstance(s, str) or len(s) != 10:
        return False
    if s[4] != "-" or s[7] != "-":
        return False
    return all(c.isdigit() for c in s[0:4] + s[5:7] + s[8:10])
...
class HaltVault(gl.contract.Contract):
    owner: Address
    guardian: Address
    ...
    def _guardian_contract(self):
        if self.guardian.as_hex == ZERO_ADDRESS:
            return None
        return gl.contract.get_at(self.guardian)
    ...
    def _frozen_live(self, a: Address) -> bool:
        """Freeze state = local flag OR live guardian status (RC port: the
        guardian no longer emits; the vault derives it from source of truth)."""
        if bool(self.frozen.get(a, False)):
            return True
        if self.guardian.as_hex == ZERO_ADDRESS:
            return False
        try:
            gov = gl.contract.get_at(self.guardian)
            return bool(gov.view().is_frozen(a.as_hex))
        except Exception:
            return False
